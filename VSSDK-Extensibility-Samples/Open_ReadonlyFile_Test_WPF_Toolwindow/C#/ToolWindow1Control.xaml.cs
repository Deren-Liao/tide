...
namespace Microsoft.Samples.VisualStudio.IDE.ToolWindow
{
    ...
    public partial class ToolWindow1Control : UserControl
    {
        ...
        private void openFileTest_Click(object sender, RoutedEventArgs e)
        {
            string tmpFile = @"c:\tmp\write.cs";
            string srcFile = @"c:\tmp\WriteSampleLogs.cs";
            try
            {
                if (!File.Exists(tmpFile))
                    File.Copy(srcFile, tmpFile);
            }
            catch (IOException)
            { }
            var dte = Package.GetGlobalService(typeof(DTE)) as DTE;
            var itemOp = dte.ItemOperations;
            var window = itemOp.OpenFile(tmpFile);
